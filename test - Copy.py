...
class Life:
    ...
    def next_move(self):
        ...
        for i in range(len(self.board)):
            for j in range(len(self.board[i])):
                s = sum(self.board[i[0]][i[1]] for i in
                        [[i - 1 if i else self.height - 1, j - 1 if j else self.width - 1],
                          [i - 1 if i else self.height - 1, j],
                          [i - 1 if i else self.height - 1, j + 1 if j != self.width - 1 else 0],
                          [i, j - 1 if j else self.width - 1],
                          [i, j + 1 if j != self.width - 1 else 0],
                          [i + 1 if i != self.height - 1 else 0, j - 1 if j else self.width - 1],
                          [i + 1 if i != self.height - 1 else 0, j],
                          [i + 1 if i != self.height - 1 else 0, j + 1 if j != self.width - 1 else 0]])
